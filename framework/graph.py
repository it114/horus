...
# There should be only node features(id, key, label, shape, color)
# and its attributes, which are obtained externally (nType, real, internal...)
class NodeS:
...
#             n2.set_attribute(ATTR_DESCRIPTOR, dst_descriptor)
    ...
    def export_to_gexf(self):
        ...
        buff += "<graph type=\"dynamic\">\n"

        buff += "<attributes class=\"node\" type=\"dynamic\">\n"
...
#         buff += "</attributes>\n"
    ...
    def get_current_permission_level_node_count(self, permission_level):
        count = 0
        for node in self.nodes_id.keys():
            if self.nodes_id[node].get_attribute(ATTR_PERM_LEVEL) \
                    == permission_level:
                count += 1
        return count
    ...
    def get_current_protected_node_count(self):
        count = 0
        for node in self.nodes_id.keys():
            if self.nodes_id[node].get_attribute(ATTR_PERM_LEVEL) is not None:
                count += 1
        return count
